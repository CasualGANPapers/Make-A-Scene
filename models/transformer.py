...
class SelfAttention(nn.Module):
    ...
    def calculate_attention(self, q, k, mask):
        ...
        if self.cogview_pb_relax and not self.rudalle_relax:
            ...
            attn_scores_scaled_max, _ = attn_scores_scaled.detach().view([attn_scores.shape[0], attn_scores.shape[1], -1]).max(dim=-1)
            attn_scores_scaled_max = attn_scores_scaled_max[..., None, None].expand([-1, -1, attn_scores.size(2), attn_scores.size(3)])
    ...
    def forward(self, x, mask, use_cache=False, cache=None):
        ...
        context = context.permute(0, 2, 1, 3).view(context.shape[0], context.shape[1], context.shape[2]*context.shape[3])
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, mask, cache=None, use_cache=False, mlp_cache=False):
        ...
        if use_cache and cached:
            mlp_out = torch.cat((cache[-1] if mlp_cache else ln_out[..., :cached, :], self.mlp(ln_out[..., :cached, :])), dim=-2)
            if mlp_cache:
...
class GPT(nn.Module):
    ...
    def _create_mask(self, text_length, image_tokens_per_dim=16):
